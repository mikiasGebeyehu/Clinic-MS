...
if (isset($_POST['signin'])) {
    ...
    if (count($error) == 0) {
        ...
        $hashed_password = $password; // Hashing the password
        $status = "Pending";
        $stmt = $conn->prepare("INSERT INTO pharmacist (id, fullname, password, email, phonenumber, status) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssss", $id, $username, $hashed_password , $email, $phono, $status);
...
<body>
<form class="mx-auto" action="sign-upp.php" method="post">
    <h2 class="text-center">Sign up for Pharmacist</h2>
    ...
    <p>I already have an account: <a href="pharmacist.php">Click here!!!</a></p>
</form>
